...
@testset "NaN and Inf" begin
    ...
    img = Observable(colorview(RGB, Ar))
    cmin, cmax = RGB(0.1,0.1,0.1), RGB(0.9, 0.9, 0.9)
    clim = Observable(CLim(cmin, cmax))
    enabled, histsigs, imgcsig = ImageView.prep_contrast(img, clim)
    imgc = imgcsig[]
