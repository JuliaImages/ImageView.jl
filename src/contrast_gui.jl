...
function contrast_gui(enabled::Signal{Bool}, hists::Vector, clim::Signal{CLim{T}}) where {T<:AbstractRGB}
    ...
    bindmap!(clim, x->change_red(clim, x), rsig, x->red_clim(x); initial = false)
    bindmap!(clim, x->change_green(clim, x), gsig, x->green_clim(x); initial = false)
    bindmap!(clim, x->change_blue(clim, x), bsig, x->blue_clim(x); initial = false)
    cguis = []
